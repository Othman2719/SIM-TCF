...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    AsyncStorage.setItem('tcf_users', JSON.stringify(state.users));
  }, [state.users]);

  // Save current user to AsyncStorage
  useEffect(() => {
    if (state.currentUser) {
      AsyncStorage.setItem('tcf_current_user', JSON.stringify(state.currentUser));
    } else {
      AsyncStorage.removeItem('tcf_current_user');
    }
  }, [state.currentUser]);
